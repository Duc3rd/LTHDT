...
public class StudentDao {
    ...
    public void insertStudent(Student student) {
        String SQL = "INSERT INTO lthdtbtl.student(id, name, dob, address_province, province) VALUES(?, ?, ?, ?, ?)";
    ...
    public List<Student> getStudentsById(int studentId) {
        List<Student> students = new ArrayList<>();
        String SQL = "SELECT * FROM lthdtbtl.student WHERE id = ?";
        try (Connection conn = connect();
             PreparedStatement pstmt = conn.prepareStatement(SQL)) {
    ...
    public List<Student> getStudentsByName(int studentName) {
        List<Student> students = new ArrayList<>();
        String SQL = "SELECT * FROM lthdtbtl.student WHERE name = ?";
        try (Connection conn = connect();
             PreparedStatement pstmt = conn.prepareStatement(SQL)) {
    ...
    public void updateStudent(Student student) {
        String SQL = "UPDATE lthdtbtl.student SET name = ?, dob = ?, addressProvince = ?, province = ? WHERE id = ?";
        try (Connection conn = connect();
             PreparedStatement pstmt = conn.prepareStatement(SQL)) {
    ...
    public void deleteStudent(int studentId) {
        String SQL = "DELETE FROM lthdtbtl.student WHERE id = ?";
        try (Connection conn = connect();
             PreparedStatement pstmt = conn.prepareStatement(SQL)) {
            pstmt.setInt(1, studentId);
            pstmt.executeUpdate();
